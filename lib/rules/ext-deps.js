...
module.exports = function(context) {
	...
	var addDependencyNode = function(node, key, allowArray) {
		if (node.type === "Literal" && typeof node.value === "string") {
			addDependency(node.value);
		}
		else if (node.type === "ArrayExpression" && allowArray) {
			node.elements.forEach(function(element) {
				addDependencyNode(element, key, false);
			});
		}
		else {
			context.report(
					...
					if (arg.type === "ObjectExpression") {
						...
						arg.properties.forEach(function(prop) {
							...
							if (key === "requires" || key === "uses") {
								addDependencyNode(prop.value, key, true);
							}
						});
					}
				}
